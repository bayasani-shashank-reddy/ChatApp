...
(function migrate() {
    if (!localStorage.getItem('chat_user') && localStorage.getItem('user')) {
        localStorage.setItem('chat_user', localStorage.getItem('user'));
        console.log("Auto-migrated 'user' to 'chat_user'");
    }
})();

// Uses BACKEND_URL from config.js when deployed on Vercel (pointing to Render).
// Falls back to relative path when running locally on the same server.
const API_BASE_URL = (typeof BACKEND_URL !== 'undefined' && BACKEND_URL && !BACKEND_URL.includes('YOUR-RENDER')) ? BACKEND_URL + '/api' : '/api';
...
const Auth = {
    ...
    getUser: () => {
        let userStr = localStorage.getItem('chat_user');
        if (!userStr) {
            // Migration from old key 'user' to 'chat_user' if it exists
            const oldUser = localStorage.getItem('user');
            if (oldUser) {
                localStorage.setItem('chat_user', oldUser);
                userStr = oldUser;
                console.log("Migrated user data to chat_user");
            }
        }
